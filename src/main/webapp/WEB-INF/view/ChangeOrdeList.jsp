<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ page import="techermanager.pojo.FightInfo" %>
<%@ page import="java.util.List" %>
<%@ page import="techermanager.dao.VO.FightInfoVO" %>
<!DOCTYPE html>
<html lang="zh-CN">
<head>


    <title>航班查询</title>

    <%@include file="layout/header.jsp" %>


</head>

<body>

<div class="container">
    <%@include file="layout/nav.jsp" %>
    <div class="row">

        <div class="col-lg-10 col-lg-offset-1">
            <h2 class="text-center form-signin-heading">改签结果列表</h2>
            <p class="text-center">出发时间:<input id="date"/></p>
            <script>
                //执行一个laydate实例
                laydate.render({
                    elem: '#date' //指定元素
                });

                function changeTicket(fightId, detailId) {

                    var da = $("#date").val();

                    if (da==''){
                        alert("请选择出发时间");
                    }else {

                        $.ajax({
                            url: "/dochangeOrder?fightId=" + fightId+"&detailId="+detailId+"&date="+da,
                            type: "get",
                            success: function (data) {
                                alert("改签成功！");
                                location.href="/user/center";
                            }
                        })
                    }
                }


            </script>
            <%
                Long detailId = (Long) request.getAttribute("detailId");
                List<FightInfoVO> fightInfos = (List<FightInfoVO>) request.getAttribute("fightInfos");
                if (fightInfos != null || fightInfos.size() != 0) {

                    for (FightInfoVO fightInfo : fightInfos) {
            %>

            <p>航班号<%=fightInfo.getFightNo()%>：
                起始地：<%=fightInfo.getDeparture()%>
                出发时间：<%=fightInfo.getDepartureTime()%>
                目的地：<%=fightInfo.getDestination()%>
                到达时间：
                <%=fightInfo.getDestinationTime()%>
                航空公司:<%=fightInfo.getFightCompany()%>
                票价<%=fightInfo.getFightPay()%>
                    <button class="btn btn-info" onclick="changeTicket(<%=fightInfo.getFightId()%>,<%=detailId%>)">改签
                </button>
            </p>


            <%
                    }
                }
            %>


        </div>

    </div>

</div>
<%@include file="layout/footer.jsp" %>
</body>
</html>

